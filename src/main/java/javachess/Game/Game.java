...
public class Game {
    ...
    public boolean move(Spot from, Spot to) {
        ...
        if (piece.getType() == PieceType.PAWN && Math.abs(from.getY() - 
                to.getY()) == 2) {
            enpassant = from.getX();
        } else {
            enpassant = -1;
        ...
        if (piece.getType() == PieceType.PAWN) {
            if (from.getX() != to.getX() && board[to.getX()][to.getY()] == 
                    null) {
                if (to.getY() == 2) {
    ...
    public boolean checkMate() {
        for (Spot spot : getPiecesOnBoard()) {
            if (board[spot.getX()][spot.getY()].isWhite() == whiteToMove) {
                if (getPotentialMoves(spot).size() > 0) {
                    return false;
                }
            }
        }
    ...
    public ArrayList<Spot> getPotentialMoves(Spot spot) {
        ...
        if (board[x][y].getType() == PieceType.PAWN) {
            ...
            for (int dx = -1; dx < 2; dx++) {
                ...
                if (newspot.onBoard()) {
                    if (dx == 0) {
                        ...
                        if ((spot.getY() == 1 && !whiteToMove) || (spot.getY() 
                                == 6 && whiteToMove)) {
                        ...
                        if (board[x + dx][y + dir] != null && 
                                board[x + dx][y + dir].isWhite() != isWhite) {
                            moves.add(newspot);
                            if (board[x + dx][y + dir].getType() == 
                                    PieceType.KING) {
                                checkDetected = true;
                                checked();
                            }
            ...
            for (Spot dir : dirs) {
                Spot newspot = new Spot(spot.getX() + dir.getX(), 
                        spot.getY() + dir.getY());
                if (!newspot.onBoard()) {
                    continue;
                }
        ...
        for (Iterator<Spot> iterator = moves.iterator(); iterator.hasNext();) {
            ...
            for (Spot piece : getPiecesOnBoard()) {
                ...
                if ((!whiteToMove && whiteChecked) || 
                        (whiteToMove && blackChecked)) {
                    remove = true;
                    break;
                }
    ...
    private boolean detectAndExecuteCastling(Spot from, Spot to) {
        if (board[from.getX()][from.getY()] == null || 
    ...
    private boolean castling(final boolean white, final boolean kingSide) {
        ...
        if (kingSide && white) {
            x = 5;
            y = 7;
            if (!whiteKingCastling || whiteChecked) {
                return false;
            }
        } else if (!kingSide && white) {
            x = 1;
            y = 7;
            if (!whiteQueenCastling || whiteChecked) {
                return false;
            }
        } else if (kingSide && !white) {
            x = 5;
            y = 0;
            if (!blackKingCastling || blackChecked) {
                return false;
            }
        } else if (!kingSide && !white) {
            x = 1;
            y = 0;
            if (!blackQueenCastling || blackChecked) {
                return false;
            }
        }
    ...
    private ArrayList<Spot> generateMovesFor(final Spot spot, final PieceType type) {
        ArrayList<Spot> moves = new ArrayList<>();
        boolean isWhite = board[spot.getX()][spot.getY()].isWhite();
        ArrayList<Spot> dirs = new ArrayList<>();
    ...
    private Spot[] generateKnightMoves(final Spot spot) {
        Spot[] moves = new Spot[8];
        for (int i = 0; i < 8; i++) {
            moves[i] = new Spot(spot.getX() + knightMoves[i].getX(), spot.getY()
                    + knightMoves[i].getY());
        }
        return moves;
    }
